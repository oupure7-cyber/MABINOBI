...
7. 🟡 앱 UI 구현 — PySide6 대시보드(`50_APP`). 스탯/재화 조회, 음악 재생/악기 변경 + "가공 무한" 조건 기반 자동 루틴(강철괴/목재+/옷감+/가죽+ 4개 체인을 한 루프가 라운드로빈으로 순회, F1 채집 + F3 가공 트리거를 재료 버퍼 규칙으로 일반화) + JOB 대기열(유한한 자동화 작업을 여러 개 순서대로 반복 실행 — 카탈로그: "가공무한 1시간", "야채볶음10개"/"야채볶음 50개", 채집 JOB 25종. 기존 채집 바로가기 버튼 그리드는 제거하고 전부 JOB으로 이전)까지 완료. 정령의 날개 하루 소모 상한 설정, 게임 채팅 전송 UI는 아직 없음(다음 단계)
