...
- 로컬 0.9.5 개발판을 0.9.6으로 올려 배포합니다.
- 채집 참고 목록을 25종에서 128종으로 확대하고 이름이 일치하는 아이콘을 추가했습니다. 채집 목록 동기화로 현재 캐릭터의 도구·숙련도 조건을 확인합니다.
- 채집·요리·제작 목록에 소지 / 창고 / 합계 수량을 밝은 노란색으로 표시합니다. 창고는 현재 캐릭터 창고와 계정 창고를 한 번씩 합산하며, 마우스를 올리면 창고별 수량을 볼 수 있습니다.
- 재고가 갱신되어도 선택 행, 스크롤 위치, 제작 수량 입력을 유지합니다. 작업으로 조회가 밀리면 마지막 확인값과 시각을 유지하며, 캐릭터 식별 중이거나 조회 실패 시 확인 불가로 표시합니다.
- 확인 전·미지원 수량은 0 대신 —로 표시합니다. 커넥터가 제공하지 않는 장비 보유 수량은 확인할 수 없습니다.
- 잘못된 조회 응답이 빈 재고로 저장되지 않도록 캐릭터 식별·조회 결과 처리를 보완했습니다.
- 검증: 자동 테스트 240개, 독립 실행 파일의 화면 기동·채집 128종 아이콘·드래그·내장 버전 검사. 이번 배포 검증에서 실제 재료를 소비하는 게임 동작은 실행하지 않았습니다.

## v0.9.4 — 2026-09-21

- 현재 로컬 개발판을 원격 변경과 통합해 배포합니다. 제작 목록 108종, 목표 수량 입력과 드래그 대기열을 포함합니다.
- 동명 제작법을 비교하고, 부족 재료의 채집·제작·시설 가공·회수 후 최종 제작을 진행하는 흐름을 포함합니다. 필요한 만큼 시설 슬롯을 채우고, 생산 대기 중에는 준비 가능한 다른 부족 재료 작업을 진행합니다.
- 캐릭터 관리·창고 검색·주간 장비 제작·보유 스크롤 처리와 계열별 가공 목표 등급 조절을 함께 제공합니다.
- 테두리 없는 80% 생산 오버레이와 위치·날씨·에린 시간 표시를 통합했습니다. 환경 조회는 화면을 막지 않도록 별도 스레드에서 진행합니다.
- 버전을 `0.9.3`에서 `0.9.4`로 올립니다. GitHub 릴리즈 자산명은 `MabiNobi.exe`이며, 로컬 실행 파일명은 `마비노비.exe`를 유지할 수 있습니다.
- 현재 현대식 화면에서는 자동 업데이트 확인이 시작되지 않으므로 새 버전은 릴리즈의 실행 파일을 직접 내려받아 교체해야 합니다. 이 릴리즈에서는 기존 제작·가공 동작을 그대로 유지합니다.
- 검증: 자동 검사 199개 통과, 독립 실행 파일 정상 기동, 제작 목록 108종 표시와 내장 버전 `0.9.4`를 확인했습니다. 실제 게임에서 재료를 소비하는 제작은 이번 릴리즈 검증 범위에 포함하지 않습니다.

## 2026-09-20 (가공 무한 알고리즘 N단계 범용화 + 목표 등급 슬라이더)

- **배경**: "가공 무한" 루틴이 지금까지 금속/목재/가죽/옷감 4계열 각각 2단계(원재료→1차
  가공물→강철괴류)에서 멈추도록 고정돼 있었는데, 실제 게임엔 계열마다 그 위로 5~6단계가 더
  있음(사용자가 전체 레시피를 불러줌 - `10_RESEARCH/05_altering_full_recipe_chains_2026-09-20.md`).
  사용자 요청으로 "어느 등급까지 만들 것인지"를 실시간으로 조절할 수 있는 범용 알고리즘으로
  재설계.
- **데이터 모델 전면 교체**(`altering_routine.py`): `Chain`/`IntermediatePath`(2단계 전용) →
  `Family`/`Tier`/`RecipeOption`/`Ingredient`(N단계) — `FAMILIES`가 4계열 각 7단계를 전부
  담음(금속 0단계만 원재료 루트 2개, 나머지는 전부 `결과물[n] <- 결과물[n-1] xN + 전용
  원재료 xM + 계열 공통 보조재료 xK`).
- **승격 알고리즘**(`_plan_one_slot`): 슬롯 하나를 채울 때 목표 등급부터 0단계까지 내려가며
  "지금 만들 수 있는 가장 높은 단계"를 찾음. 목표 등급 미만의 모든 단계는 "그 산출물이 다음
  단계 대기열 한 바퀴(7슬롯) 분 + 5개를 넘어설 때만 초과분을 상위 합성에 쓴다"는 예약 규칙이
  걸려있고, 목표 등급 자신은 예약 없이 무제한 축적 — 사용자가 명시한 "필수 개수보다 훨씬
  초과하면 나머지는 전부 상위 재료로 승격" 요청 그대로 구현. 원자재 자동 채집 범위는 기존과
  동일(3단계 이상에서 새로 필요한 전용 원재료/보조재료는 1차 버전에서 자동 채집 안 함 —
  보유분만 소비, 부족하면 하위 단계로 자동 폴백).
- **새 UI**(`tier_target_control.py`, `routine_dashboard.py`): 계열마다 `QSlider` 기반
  목표 등급 슬라이더(`<-----●----->`, 사용자 스케치) - 7개 단계 이름을 슬라이더 아래 라벨로
  항상 표시하고 현재 선택만 강조(사용자 지적 - "노드 위치가 뭘 뜻하는지 잘 모를테니 산출물
  이름을 표기해달라"). `routine_dashboard.py`의 기존 "재료/생산물 현황" 테이블은 삭제하고 그
  자리에 이 슬라이더 4개를 배치. `wire_targets(worker)`가 슬라이더 값을 워커에 실시간
  반영 - "가공무한 1시간" JOB처럼 같은 대시보드 위젯이 여러 워커 인스턴스를 번갈아 상대하는
  경로에서도 재연결 시 이전 워커에는 더 이상 반영되지 않도록 처리.
- **성능**: 재고 조회(`_inventory_counts`)를 이름별 개별 `get_items` 호출(계열당 최대 15개
  이름 × 4계열 ≈ 50회) 대신 필터 없는 호출 1번 + 로컬 집계로 변경 - 7단계로 늘면서 이름 수가
  늘어도 라운드당 CLI 호출 수는 그대로.
- 유닛 테스트 108개(신규 19개: 데이터 모델 무결성, 승격/예약/폴백 로직, 슬라이더 연동) 전부
  통과. 오프스크린으로 "무한가공소" 탭의 슬라이더 4개 정상 렌더링 및 기본값(강철괴/목재+/
  가죽+/옷감+) 확인, 슬라이더↔워커 실시간 연동을 워커 직접 생성으로 검증(실제 CLI 없이도
  가능한 범위).

## 2026-09-20 (인게임 오버레이 HUD 추가 + 위치 계산 버그 수정)

- **오버레이 HUD 신규**(`app/dashboard/overlay.py`, 사용자 요청): 새로고침↔캐릭터 관리 사이 "오버레이" 토글 버튼(체크형) — 게임이 이미 연결돼 있으면 시작 시 자동 on, 연결되는 순간에도 자동 on, 앱 종료 시 정리. 게임 창 위치는 `ctypes`로 `FindWindowW`+`GetClientRect`+`ClientToScreen`만 사용해 읽고(신규 의존성 없음), 게임 프로세스 자체는 절대 건드리지 않음. 모든 패널은 `Qt.WindowTransparentForInput`으로 마우스/키보드 입력이 항상 아래로 통과하고, 배경은 반투명(`WA_TranslucentBackground`). 0.5초마다 게임 창 rect를 다시 읽어 재배치 — 사용자가 게임 창을 옮기거나 리사이즈해도 계속 따라감. 패널 2종: ①좌중간 "가공 시설" 4개 상태(최대한 작게), ②중상단 인게임 날짜/시간/날씨/위치.
- **사용자 실측 리포트 — 위치가 틀어짐**: 게임 창을 리사이즈한 뒤 확인해보니 환경 정보 패널이 화면 우상단(의도: 게임 창 중상단), 가공 패널이 화면 좌하단 쪽(의도: 게임 창 좌중간)에 나타남. 원인: 각 패널이 `adjustSize()`로 "현재 라벨 텍스트 기준" 크기를 잡은 뒤 그 크기로 위치를 계산했는데, 매 틱마다 위치 계산(`reposition`)이 데이터 갱신(`refresh_data`)보다 먼저 실행돼서 아직 placeholder 텍스트("-/7", "-")로 그려진(실제보다 작은) 크기를 기준으로 좌표를 잡고 있었음 — 실제 데이터가 들어간 뒤 더 길어진 텍스트와 계산이 어긋나 배치가 틀어짐. 안티치트가 외부 프로세스의 `SetWindowPos` 호출 자체를 막고 있어(`GetLastError=5`, ERROR_ACCESS_DENIED로 실측 확인) 사용자의 리사이즈 상황을 이 쪽에서 직접 재현할 수는 없었음 — 코드 리뷰로 원인을 특정.
- **수정**: 패널 크기를 매번 콘텐츠에 맞춰 동적으로 재는 대신 `PANEL_SIZE`로 고정 — 위치 계산이 항상 게임 창 rect에만 의존하고 현재 라벨 텍스트 길이와는 무관해짐. 매 틱 순서도 데이터 갱신 → 위치 계산 → 표시로 정리.
- **가독성 개선**(사용자 요청 — "너무 잘 안보인다", background 도형/이모지 허용): 배경 불투명도를 20%→62%로 올려 텍스트 대비 확보, 각 패널에 둥근 테두리 배지 스타일 적용. 가공 시설 4줄에 이모지(⚙️금속/🪵목재/🟤가죽/🧵옷감) + 7칸 유니코드 진행 막대(`▰▱`, 완료분은 밝은 초록으로 구분) + 완료 개수(✓) 표시로 교체. 환경 정보 패널에 위치(📍)/날씨(이모지 매핑, 알 수 없는 값은 기본 이모지로 대체)/시간(🕐) 아이콘과 구분선 추가.
- 유닛 테스트(`test_overlay.py`) 8개 포함 전체 89개 통과.

## 2026-09-20 (주간 제작 수량 수정 — 스크롤 주간 구매 한도 3개 반영)

- **사용자 지적**: 임무 게시판은 같은 스크롤을 한 주에 최대 3개까지 팔 수 있는데(스크롤 1개 = 장비 2개 목표), 어제 만든 "주간 제작(마을)" 버튼은 스크롤 1개 몫(x2)만 대기열에 넣고 있었음 — 실제로는 x6(3×2)이어야 함.
- `job_queue.py`에 숨은 JOB 두 종류 추가: `EQUIPMENT_WEEKLY_X6`(장비 3종 × x6, "주간 제작(마을)" 버튼용), `EQUIPMENT_WEEKLY_X5`(x5, "주간 제작(마을) x5" 버튼용 — 기존 `EQUIPMENT_WEEKLY_X10`과 같이 큐에 넣어 10+5=15를 채움. 새 x15 JOB을 따로 만드는 대신 이미 만든 x10을 재사용하기로 사용자가 지정).
- `modern_window.py`의 `add_weekly_equipment()`를 이 두 JOB을 쓰도록 수정. `quantity()`도 함께 손봄 — 기존엔 '제작' 카테고리를 `spec.key` 접두사(`equipment_x10_`)로만 구분해 하드코딩했는데, 새 x6/x5까지 늘어나며 분기가 늘어나는 대신 `spec.make_worker().remaining`(막 생성한, 아직 시작 안 한 워커가 이미 들고 있는 진짜 목표값)을 그대로 읽도록 일반화 — 앞으로 어떤 x-배수 변형이 추가돼도 `quantity()`를 다시 고칠 필요가 없어짐.
- 유닛 테스트 81개(신규 4개) 전부 통과. 오프스크린으로 "주간 제작(던바튼)" → 3종 모두 x6, "주간 제작(반호르) x5" → 3종 모두 x10+x5(합계 15) 큐 항목 확인.

## 2026-09-20 (네 번째 릴리즈: GitHub Release v0.9.3 게시 — 제작 탭 마을별 스크롤/배치 경제화/보유 스크롤 진행 반영)

- `APP_VERSION`을 `0.9.3`으로 올리고 exe 재빌드, 유닛 테스트 79개 + `--ui-self-test` 통과 후 GitHub Release `v0.9.3` 게시(자산명 `MabiNobi.exe`, ASCII)
- 반영된 내용: 바로 아래 두 항목("보유 스크롤 모두 진행" 구현 등, 제작 탭 마을별 주간 스크롤 버튼 + execute_crafting 배치 경제화) 전체

## 2026-09-20 ("보유 스크롤 모두 진행" 구현 + x10 JOB 수량 표시 버그 수정 + "엣지소드" 표기 통일)

- **x10 JOB 수량 표시 버그 수정**(사용자 발견): `modern_window.py`의 `quantity()`가 '제작' 카테고리는 무조건 `2 * repeats`로 계산해서, 어제 만든 x10 히든 JOB(주간 x5 버튼용)도 대기열에서 "목표 2개"로 잘못 표시되고 있었음(실제 제작 개수 자체는 정상). 요리 카테고리가 10/50을 `spec.key`로 구분하는 것과 같은 방식으로 `equipment_x10_` 접두사를 확인하도록 수정.
- **제작 탭 안내 문구 교체**(사용자 요청): "한 작업당 장비 2개 제작 · ..." → "오른쪽 대기열로 드래그하세요. 요청 1회마다 정령의 날개 5개 소모. x5 스크롤이 가장 경제적입니다!\n별도 가공/구매 재료가 부족하면 안내 후 멈춥니다."
- **"보유 스크롤 모두 진행" 구현**(어제는 자리만 마련해둔 버튼): 스크롤은 창고(캐릭터창고/계정창고) 보관이 안 되는 아이템이라 인벤토리만 확인 — `get_items`를 "제작 스크롤: " 부분일치로 조회해서(`ScrollInventoryWorker`), 인벤토리에 있는 "제작 스크롤: `<이름>`"을 아는 레시피(`EQUIPMENT_RECIPES`)와 매칭되면 보유 개수만큼 반복(◀N▶, 1~9 상한)으로 대기열에 추가. 아직 JOB으로 안 만든 다른 종류의 제작 스크롤은 조용히 건너뜀(사용자 지시).
  - **실측으로 발견한 표기 불일치**: 실제 보유 스크롤 14종을 조회해보니 "제작 스크롤: 론 엣지소드S"(공백 없음)인데 카탈로그의 `EQUIPMENT_RECIPES`엔 "론 엣지 소드S"(공백 있음)로 등록돼 있어서 정확히 일치 비교로는 매칭이 안 되는 문제 발견 — `get_craftable_items` 응답 매칭에 이미 쓰던 것과 같은 공백 정규화 매칭(`_find_equipment_spec`)을 스크롤 매칭에도 적용해서 해결. 실제 인벤토리(스크롤 14종 중 12종이 아는 레시피, 2종은 미지원 제작 종류 — "전문 캠프파이어 키트", "마법 유탄 부품")로 종단 검증 완료, 12종 전부 정확히 매칭·대기열 추가 확인.
  - 사용자 지적으로 "엣지 소드"(공백 있음) 표기가 게임 내 정확한 표기(공백 없는 "엣지소드")와 다르다는 것을 확인 — `EQUIPMENT_RECIPES`/`TOWN_EQUIPMENT`의 "론 엣지 소드S"를 "론 엣지소드S"로 전부 수정(테스트 포함).
- 유닛 테스트 79개(신규 9개) 전부 통과.

## 2026-09-20 (제작 탭: 마을별 주간 스크롤 버튼 + execute_crafting 배치 경제화)

- **배경**: 장비 제작은 마을(티르코네일/던바튼/콜헨/반호르/이멘마하 등) 임무 게시판 NPC에게 사는 "제작 스크롤: `<이름>`" 퀘스트 아이템 기반이고, 스크롤 하나당 장비 2개가 목표(어제 제작 목표를 3→2로 바꾼 이유). 마을별로 파는 스크롤 3종: 던바튼(크레센트 엣지소드/그랜드 크로스보우/로터스 힐링 완드), 콜헨(두꺼운 전투복 신발/사슬 갑옷 신발/두꺼운 가죽 갑옷 신발), 반호르(론 엣지 소드S/라이트 크로스보우S/마블 힐링 완드S), 이멘마하(가죽 갑옷 신발S/전투복 신발S/비늘 갑옷 신발S) — `equipment_crafting.py`의 `TOWN_EQUIPMENT`.
- **제작 탭 새 버튼**(검색창 위, `modern_window.py`): "보유 스크롤 모두 진행"(동작은 추후 지정 예정, 지금은 자리만), 마을별 "주간 제작(마을명)" 4개(그 마을 3종을 x2로 대기열 추가), "주간 제작(마을명) x5" 4개(같은 3종을 5주치(x10)로 한 번에 추가 — 목록엔 없는 숨은 JOB, `job_queue.py`의 `EQUIPMENT_WEEKLY_X10`).
- **execute_crafting 배치 경제화**(사용자 지적 — "5회 연속으로 만들면 x2회 제작을 끊어서 전달하는 모양"): `execute_crafting`은 `craftCount`(1회 호출당 제작 반복 횟수)와 무관하게 호출 1번당 정령의 날개를 소모하는데, 기존 코드는 항상 `craftCount:1`로만 호출해서 목표 수량만큼 호출을 반복 — 예를 들어 장비 2개(생산량 1개/회)면 2번 호출해 10개(정령의 날개 10개)를 써야 할 걸 그대로 두면 5주치(10개)를 만들 때 10번(50개) 호출하는 구조였음. `recipe_cooking.py`에 `craft_batch()`를 새로 추가 — 시설이 허용하는 한 목표 수량 전체를 한 번의 `craftCount`로 요청하고, 시설별 상한은 `invalid_count`+`maxCount` 응답으로 실측 학습해 필요할 때만 나눠 재시도(`food_crafting_job.py`가 이미 쓰던 방식과 동일). 여러 번 호출로 쪼개지는 도중 실패해도 이미 성공한 만큼은 `self.remaining`에 즉시 반영되어 유실되지 않음. 기존 `craft_one()`/`ensure()`(하위 재료 재귀 준비용)는 그대로 둬서 영향 범위를 이 최상위 목표 수량 크래프팅으로 한정.
- 유닛 테스트 70개(신규 8개) — 배치 호출 횟수 검증(균등분할 시 단일 호출, 시설 상한 학습·재사용, 부분 성공 후 실패 시 진행량 유지)까지 포함해 전부 통과. 오프스크린/`--ui-self-test`로 버튼 배치·대기열 추가 동작 스크린샷까지 확인.
