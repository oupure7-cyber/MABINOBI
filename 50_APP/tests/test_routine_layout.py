...
class RoutineLayoutTests(unittest.TestCase):
    ...
    def test_snapshot_updates_both_material_columns_without_losing_names(self):
        ...
        self.assertEqual(self.dashboard._table.rowCount(), 8)
    ...
    def test_unknown_is_not_displayed_as_empty_until_first_snapshot(self):
        slots = self.dashboard._queue_slots['steel']
        self.assertEqual(slots.states, ['unknown'] * 7)
        self.dashboard._on_snapshot({'queue': {'steel': 0}})
        self.assertEqual(slots.states, ['empty'] * 7)
        self.assertEqual(self.dashboard._queue_label['steel'].text(), '등록된 작업 없음')

    def test_completed_and_unfinished_occupancy_are_separate(self):
        self.dashboard._on_snapshot({'queue': {'steel': 5}, 'queue_completed': {'steel': 2}})
        self.assertEqual(self.dashboard._queue_slots['steel'].states,
                         ['completed'] * 2 + ['occupied'] * 3 + ['empty'] * 2)
        self.assertEqual(self.dashboard._queue_count['steel'].text(), '5 / 7')
        slots = FacilitySlots()
    ...
    def test_blocked_hides_stale_counts_and_ignores_later_snapshots(self):
        ...
        self.dashboard._on_snapshot({'queue': {'steel': 3}})
        ...
        self.assertEqual(self.dashboard._queue_count['steel'].text(), '3 / 7')
