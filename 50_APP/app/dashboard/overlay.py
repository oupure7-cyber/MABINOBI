...
from PySide6.QtCore import QObject, QRect, Qt, QTimer, QThread, Signal
...
class EnvironmentOverlay(OverlayWindow):
    ...
    def __init__(self, parent=None, *, compact=False):
        # A parentless native window remains visible while the dashboard is
        # minimized. The owning controller handles its lifetime explicitly.
        super().__init__(None if compact else parent)
        ...
        if compact:
            self.setFixedSize(240, 35)  # 80% of the upstream 300 x 44 HUD.
            self.setWindowFlag(Qt.WindowDoesNotAcceptFocus, True)
            self.setFocusPolicy(Qt.NoFocus)
            self.setAutoFillBackground(False)
            self.setStyleSheet(
                "QWidget { background: transparent; border: none; } "
                "QLabel { color: #eaf2f0; font-family: 'Malgun Gothic'; "
    ...
    def set_data(self, data) -> None:
        if (not isinstance(data, dict) or data.get('error') or data.get('pipe') == 'disconnected'
                or not any(key in data for key in ('GameSpaceDisplayName', 'Weather', 'ErinnNow'))):
            return
        place = escape(str(data.get("GameSpaceDisplayName", "-")))
        weather = escape(str(data.get("Weather", "-")))
        when = escape(str(data.get("ErinnNow", "-")))
        place_text = f'📍 {place}' if self._compact else f'<b>📍 {place}</b>'
        html = (
            place_text + f' <span style="color:{DIM};">|</span> {_weather_emoji(weather)} {weather}'
...
class EnvironmentPollWorker(QThread):
    result = Signal(object)

    def run(self):
        # try_run_cli returns immediately if the shared CLI lock is busy.
        # A slow subprocess can wait here without blocking the Qt GUI thread.
        try:
            data = try_run_cli('get_current_environment', timeout=10)
        except Exception:
            data = None
...
class EnvironmentOverlayController(QObject):
    """The upstream environment HUD beside the existing progress overlay.

    This owns no facility panel. Position tracking uses the progress overlay's
    foreground-game/DPI source; read-only CLI polling lives in one QThread.
    """
    def __init__(self, game_source, parent=None, *, polling_allowed=True):
        super().__init__(parent)
        self.panel = EnvironmentOverlay(compact=True)
        self.destroyed.connect(self.panel.deleteLater)
        self.game_source = game_source
        self.polling_allowed = polling_allowed
        self.worker = None
        self.enabled = False
        self._position_timer = QTimer(self)
        self._position_timer.setInterval(TRACK_INTERVAL_MS)
        self._position_timer.timeout.connect(self._position)
    ...
    def set_opacity(self, value):
        self.panel.setWindowOpacity(max(.2, min(1., float(value))))

    def _position(self):
        rect = self.game_source.rectangle() if self.enabled else None
        if rect is None:
            self.panel.hide()
            return
        if hasattr(rect, 'toRect'):
            rect = rect.toRect()
        self.panel.reposition(rect)
        self.panel.show()

    def _poll(self):
        if (not self.enabled or not self.polling_allowed or self.worker is not None
                or self.game_source.rectangle() is None):
            return
        self.worker = EnvironmentPollWorker(self)
