...
- `app/version.py` / `app/updater.py` / `app/dashboard/update_worker.py` — 버전 정보와 자동 업데이트 모듈입니다. 현재 진입 화면인 `modern_window.py`는 자동 업데이트 확인을 시작하지 않으므로 새 릴리즈의 실행 파일을 직접 받아 교체해야 합니다. 배포 절차: `APP_VERSION` 변경 → exe 재빌드·검증 → 일치하는 `vX.Y.Z` 태그로 Release 생성 → 자산을 ASCII 이름 `MabiNobi.exe`로 첨부 → 게시. 로컬 실행 파일 이름은 `마비노비.exe`를 유지해도 됩니다.
