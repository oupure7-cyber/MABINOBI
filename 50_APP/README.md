...
- 채집 참고 목록 128종을 표시하며, `채집 목록 동기화`로 실제 게임의 채집 조건을 확인합니다.
- 채집·요리·제작 목록 오른쪽의 노란색 `소지 / 창고 / 합계`로 현재 보유 수량을 비교합니다. 창고는 현재 캐릭터 창고와 계정 창고의 합계이며, 숫자에 마우스를 올리면 창고별 수량을 표시합니다.
- 게임 연결 후 자동 갱신합니다. 작업 중 조회가 밀리면 마지막 확인 시각을 표시하고, 캐릭터 전환·조회 실패·장비 등 미지원 수량은 `—`로 구분합니다.
- 테스트: `python -m unittest discover -s 50_APP/tests` (240개). 배포 EXE는 `--ui-self-test 결과.json`으로 게임 동작 없이 화면과 내장 버전을 검사할 수 있습니다.
